...
function CrewManagementGui:create_mask_button(panel, index)
	...
	return CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture}, text, cat_text, callback(self, self, "open_mask_category_menu", index))
end
function CrewManagementGui:create_weapon_button(panel, index)
	...
	local item = CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture, layer = 1} or managers.localization:to_upper_text("menu_crew_defualt"), text, cat_text, callback(self, self, "open_weapon_menu", {"primaries", index}))
	if rarity then
...
function CrewManagementGui:create_ability_button(panel, index)
	...
	return CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture, texture_rect = texture_rect}, text, cat_text, callback(self, self, "open_ability_menu", index))
end
function CrewManagementGui:create_skill_button(panel, index)
	...
	return CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture, texture_rect = texture_rect}, text, cat_text, callback(self, self, "open_skill_menu", {"skill", index}))
end
...
function CrewManagementGuiButton:_selected_changed(state)
	if self._select_panel then
		local _ = self._select_panel:set_visible(state)
	end
end
...
function CrewManagementGui:open_custom_menu(params, category, custom_w, custom_h)
	...
	function new_node_data.custom_update_text_info(data, updated_texts, gui)
		...
		if data.unlocked then
			...
			updated_texts[3].text = managers.localization:text("menu_data_crew_equipped_by", {
